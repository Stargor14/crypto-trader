...
def close(ex,pnl,row,type):
    ...
    print(f"Total balance: {balance} Total profit: {profit}% Total trades: {tradesum} Good longs: {goodlong} Bad longs: {badlong} Good shorts: {goodshort} Bad shorts: {badshort}")
    if pnl <0:
        if type == 'l':
            badlong+=1
        else:
